...
def x8M(r, m1, m2, m3, e1, e2, e3, c):
    ...
    x = 0
    #
    if m1 != 0: x = np.maximum( (m1-r)/m1, x)
    #
    if ma <= 0 and mb > 0:
        x = np.maximum( 
            h * np.minimum( (m3 -r)/mb, 1),  
            x )
    elif ma > 0 and mb <= 0:
        x = np.maximum(
                h * np.minimum( 1, 
        ...
        x = np.maximum(
                h * np.minimum( 
                    np.minimum( (m3 -r)/mb, 1), 
                    (r - m1e)/ma 
                ), x )   
    #      
    if m3e < 1:
        x = np.maximum((r - m3e)/(1 - m3e), x)
    return x
...
def x8Dome(r, m1, m2, m3, e1, e2, e3, c):
    ...
    x = 0
    #
    if m1 != 0: x = np.maximum( (m1-cr)/m1, x)
    #
    if ma <= 0 and mb > 0:
        x = np.maximum( h * np.minimum((r - m2e)/mb, 1), x)
    if ma > 0 and mb <= 0:
        x = np.maximum( h * np.minimum( (m2-r)/ma, 1), x)
    elif ma > 0 and mb > 0:
        x = np.maximum( 
                h * np.minimum(
                        np.maximum((r - m2e)/mb, 
...
def glin(x, *, p=1):
    ...
    Spec: 
        - x is a real number or an array-like data structure in [0, 1]
