...
This repository provides implementation of learning-rate schedules built from a boundary-value problem (BVP) profile. The schedule design follows a normalized training horizon `r in [0,1]`, where the profile captures the desired shape of the expected step-size magnitude over the training horizon, and a smooth variational window function (a linear window) or its regularized version (a raised-cosine window) converts that profile into a practical expected step-size curve for the learning-rate that decays at the end of the learning horizon, typically to zero.
...
into the annealing operation. The schedule is then generated by passing the profile into a window function `y(x(r))`.

Optionally, if the user decides to not decay to 0. The schedule is then passed into `endpt(y(x(r)), l)` to decay to a `l in [0,1)` fraction of the initial step-size estimate `mu`
...
*Goal*: Minimize the Total first-order variational energy functional subject to boundary value constraints.
...
This is the canonical shape obtained on minimizing a total first-order variational energy functional of the expected step-size magnitude.
...
class BVP_LRS:
    """
    TRUE wrapper: modifies the step-size of a PyTorch optimizer
